...
class display {
	...
	public void scoreDraw(score s, Graphics g) {
		...
		f = new Font("メイリオ" , Font.BOLD , 20);
		g.setFont(f);
		g.drawString("最大連鎖:", BW*12, BH*4);
		...
		if(s.chainNum!=0) {
			...
			g.setFont(f);
			g.drawString(s.chainNum+"れんさ!",BW*9-fontMetrics.stringWidth(Integer.toString(s.chainNum)), BH*10);
		}
	}
	
	public void drawNextBlock(field a, Graphics g, JPanel p) {
		//queueをすべて取り出して表示し、もう一度入れ直す
